# ###################################################################
#   🎯 Projeto: InvestidorWeb - Carteira completa (Dia 27)            #
# ###################################################################
# 📁 Caminho: dia-27/dia27_app.py
# Desafio 30 dias com Python por Victor Beal
# ###################################################################
# Continua o Dia 26 (CRUD da carteira) e completa o ACOMPANHAMENTO:
#
# - Posicoes por ticker (qtd, preco medio, investido)
# - Precos atuais via Yahoo (yfinance)
# - Resumo: patrimonio, investido, lucro, rentabilidade %
# - Grafico mensal: valor aplicado x patrimonio (Chart.js)
# - API /api/carteiras/<id>/acompanhamento
#
# Herdado do Dia 26:
# - Varias carteiras + 1 padrao + lancamentos + preco na data
# - UX: InvestidorWeb titulo, toast 5s, loading, modal confirm
# ###################################################################
...
# ALTERADOS:
# - dia-27/dia27_app.py
# - dia-27/services/preco_yahoo.py   (precos atuais + historico)
...
def api_carteira_acompanhamento(carteira_id):
    """Resumo + posicoes + evolucao mensal (Yahoo)."""
    carteira = get_carteira(carteira_id, session["user_id"])
    if not carteira:
        return jsonify({"erro": "Carteira nao encontrada."}), 404
    try:
        dados = montar_acompanhamento(listar_lancamentos(carteira_id), meses_grafico=12)
