# ###################################################################
#   🎯 Projeto: Detalhe FII/Ação - Investidor Web (Dia 24)          #
# ###################################################################
# 📁 Caminho: dia-24/dia24_app.py
# Desafio 30 dias com Python por Victor Beal
# ###################################################################
# Continua o Dia 23 (Top 10 + rankings) e adiciona:
# - Detalhe ao vivo de FII e Ação (scrape sob demanda)
# - Clique no ticker + busca na navbar
# - Tudo protegido por login (sem API publica / sem key externa)
# ###################################################################
...
# --- Novidade Dia 24: busca e detalhe (somente usuario logado) ---
@app.route("/buscar")
@login_required
def buscar_ativo():
    ...
    if tipo not in TIPOS_DETALHE:
        flash("Selecione FII ou Ação.", "warning")
        return redirect(url_for("painel"))
    if not ticker:
...
if __name__ == "__main__":
    ...
    UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
    print("\n" + "=" * 50)
    print("  Investidor Web - Dia 24")
    print("  Acesse: http://localhost:5000/")
    print("  Detalhe: /ativo/fii/TICKER ou /ativo/acao/TICKER")
